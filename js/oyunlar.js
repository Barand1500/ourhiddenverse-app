...
async function loadOyunlarPage() {
  ...
  pageContent.innerHTML = `
    <div class="filmler-container oyunlar-container">
      ...
      <!-- TAŞ KAĞIT MAKAS -->
      <div class="oyun-section-card tkm-section">
        <div class="section-header-box">
          <span class="section-icon">✊</span>
          <h3 class="section-title-box">Taş Kağıt Makas</h3>
...
async function tkmSelect(player, choice) {
  if (tkmState.sonuc) return; // Sonuç varsa seçim yapılamaz
  ...
  try {
    ...
    // Her iki seçim de yapıldıysa sonucu hesapla
    const docSnap = await window.firestoreGetDoc(tkmDocRef);
